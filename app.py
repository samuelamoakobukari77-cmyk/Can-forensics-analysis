...
st.set_page_config(page_title="CAN Bus Forensic Analysis", layout="wide")
...
def generate_pdf_report(case_id: str, classified_df: pd.DataFrame, ledger: EvidenceLedger) -> bytes:
    ...
    story.append(Paragraph("Detected Incidents", styles["Heading2"]))
    ...
    ledger.add_record("export", {"report_generated_at": datetime.utcnow().isoformat(), "report_sha256": report_hash})
...
def main():
    ...
    with st.sidebar:
        ...
        if run_analysis and raw_df is not None:
            ...
            with st.spinner("Engineering features and classifying..."):
    ...
    tabs = st.tabs(["📋 Detection Results", "📈 Attack Timeline", "🔗 Chain of Custody", "🔍 SHAP Explainability", "📄 Forensic Report"])
    ...
    with tabs[4]:
        ...
        st.caption("Bundles the detection summary, incident log, and chain-of-custody status into a signed PDF.")
        if st.button("📄 Generate PDF report"):
            pdf_bytes = generate_pdf_report(st.session_state.case_id, df, st.session_state.ledger)
